...
def export_data(request):
    """
    Export data based on the provided criteria as a CSV file.

    Arguments:
        request: The Django request object.
    
    Returns:
        HttpResponse with the CSV file as the response content 
    ...
    if request.method == 'POST':
        ...
        if event == 'all':
            query = {'_id': ObjectId(request.session.get('session')),
                     'data': {'$elemMatch': {'date': {'$gte': start_date,
                     '$lte': end_date}}}}
        else:
